...
def home():
    # nah disini kita ngambil token yang di simpan di browser kita saat login
    # karena tadi kamu gagal membuat token, sehingga saat diambil nilainya kosong
    token_receive = request.cookies.get(TOKEN_KEY)
    try:
        # karena tadi tokennya kosong, sehingga saat coba dipecahkan/decode, maka dia DecodeError
        payload = jwt.decode(
            token_receive, 
            SECRET_KEY, 
            algorithms=[ALGO]
    ...
    except jwt.exceptions.DecodeError: #sehingga baris ini yang dijalankan untuk menangkap error decode nya
        msg='There was problem logging you in'
        return redirect(url_for('login', msg=msg))
    # kurang lebih gitu sih, gimana ndry, bisa dipahami? bisa kak
    # woke kalau gitu kamu bisa lanjutkan saja pengerjaan tugasnya ya, untuk any desknya silahkan di tekan tombol disconnect
